...
void MovementComponent::startRoll()
{
	...
	this->velocity.x = sgn(this->velocity.x) * this->maxRollVelocity;
	this->velocity.y = sgn(this->velocity.y) * this->maxRollVelocity;
}
...
void MovementComponent::move(const float dir_x, const float dir_y, const float& dt)
{
	...
	if (this->rolling == false) { 
		...
		this->velocity.x += (sgn(this->velocity.x) == sgn(dir_x)) ? this->acceleration * dir_x * dt * (1 / this->maxVelocityMultiplier) : 0;
		this->velocity.y += (sgn(this->velocity.y) == sgn(dir_y)) ? this->acceleration * dir_y * dt * (1 / this->maxVelocityMultiplier) : 0;
	}
